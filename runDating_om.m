...
ylabel('Subepoch')
axis tight
saveas(gcf, strcat(pwd,filesep,'figures',filesep,'KNN300_',model_identifier,'_',layer,'_',num2str(hier_level),'.pdf'));
...
% arrayfun(@(i) plot(productionTime(epoch_loc{i})', testAccSubsamples(epoch_loc{i})', 'Linewidth', 3, 'Color', colors(i,:)), [1:max(epoch)])
hold on
arrayfun(@(i) scatter(productionTime(epoch_loc{i})', mean_norm(epoch_loc{i})', 1, colors(i,:), 'filled', 'o'), [1:max(epoch)])
...
%% Vector norm and std in same plot
% figure;scatter(1:length(mean_norm),mean_norm,1);hold on;scatter(1:length(std_norm),v,1,'red');hold on;...
%     legend('Mean','Std');xticks(linspace(0,18000,7));xticklabels({0,10,20,30,40,50,60});xlabel('Epoch');ylabel('Neighborhood time distance')

figure;
ax=axes()
hold on 
scatter(productionTime', mean_norm', 1, 'red', 'filled', 'o')
hold on
arrayfun(@(i) scatter(productionTime(epoch_loc{i})', std_norm(epoch_loc{i})', 1, 'blue', 'filled', 'o'), [1:max(epoch)])
hold on
legend('Mean','Std')
hold on
ax.YLabel.String='Neighborhod time distance'
...
set(gca,'XTickLabel',downsample(h,20))
axis tight
saveas(gcf, strcat(pwd,filesep,'figures',filesep,'mean_std_norm_KNN_',model_identifier,'_',layer,'_',num2str(hier_level),'.pdf'));
